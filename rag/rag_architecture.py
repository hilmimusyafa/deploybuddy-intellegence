...
class ArchitectureRAG:
    ...
    def _build_query(
        ...
        return (
            f"{stack} {' '.join(service_types)} {target_region} "
    ...
    def recommend(
        ...
        query = self._build_query(stack, target_region, service_types, budget_usd, concurrent_users)
...
Based ONLY on the provider data in the CONTEXT above, give me the best deployment
architecture recommendation with provider comparison and JSON deployment spec.
"""
        # Step 5: LLM call
...
class ArchitectureResult:
    ...
    def __init__(self, text: str, deploy_json: Optional[Dict], retrieved_chunks: List[Dict]):
        self.text             = text
