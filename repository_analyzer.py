...
class RepositoryProfile:
    ...
    def to_json(self) -> Dict:
        return {
            ...
            "env_vars_detected": self.env_vars_detected,
    ...
    def to_context(self, max_chars: int = 18000) -> str:
        ...
        lines.extend(["", "Environment variables detected from safe examples:"])
        if self.env_vars_detected:
            lines.extend(f"- {name}" for name in self.env_vars_detected)
        else:
            lines.append("- None")
    ...
    def print_summary(self):
        ...
        if self.env_vars_detected:
            print(f"Env vars     : {', '.join(self.env_vars_detected[:12])}")
...
class RepositoryAnalyzer:
    ...
    def analyze_path(self, repo_path: str, source: Optional[str] = None) -> RepositoryProfile:
        ...
        self._skipped_sensitive_files: Set[str] = set()
        ...
        warnings.extend(sorted(state.warnings))
        if self._skipped_sensitive_files:
            skipped = ", ".join(sorted(self._skipped_sensitive_files))
            warnings.append(f"Skipped sensitive files during analysis: {skipped}")
        profile = RepositoryProfile(
            ...
            env_vars_detected=sorted(state.env_vars_detected),
    ...
    def _should_skip_file(self, path: Path) -> bool:
        ...
        if name in SENSITIVE_FILE_NAMES or suffix in SENSITIVE_SUFFIXES:
            skipped = getattr(self, "_skipped_sensitive_files", None)
    ...
    def _analyze_file(self, rel_path: str, content: str, state: "_DetectionState"):
        ...
        elif name in {".env.example", "env.example"}:
            state.env_vars_detected.update(_parse_env_example_keys(content))
            if state.env_vars_detected:
                state.stack.add("Environment Variables")
    ...
    def _detect_python_deps(self, deps: Iterable[str], state: "_DetectionState"):
        ...
        rag_deps = {"chromadb", "sentence-transformers"}
        if dep_set & rag_deps:
            state.service_types.add("model")
            state.stack.add("RAG/Vector Search")
            state.frameworks["model"].add("ChromaDB")

        llm_deps = {"groq", "openai", "google-generativeai", "anthropic"}
        if dep_set & llm_deps:
            state.service_types.add("model")
            state.stack.add("LLM API Integration")
    ...
    def _detect_source_patterns(self, rel_path: str, lower: str, state: "_DetectionState"):
        ...
        code_lower = _strip_string_literals(lower)
        if suffix in {".py"}:
            ...
            if "flask(" in code_lower or "from flask" in code_lower:
                ...
                state.service_types.add("backend")
            if "import django" in code_lower or "from django" in code_lower:
                state.runtimes.add("Python")
            ...
            if "from 'next" in lower or 'from "next' in lower or "/pages/api/" in rel_path.lower() or "/app/api/" in rel_path.lower():
                ...
                if "/api/" in rel_path.lower():
                    state.service_types.add("backend")
            if "express()" in code_lower or "require('express')" in lower or 'require("express")' in lower:
                state.runtimes.add("Node.js")
        ...
        if _looks_like_runtime_config_path(rel_path):
            self._detect_database_words(lower, state)
    ...
    def _build_architecture_hints(self, state: "_DetectionState") -> List[str]:
        ...
        if "RAG/Vector Search" in state.stack:
            hints.append("RAG/vector search dependency detected; account for embedding model downloads and persistent vector storage.")
        if state.env_vars_detected:
            hints.append("Safe environment example detected; generated deployment should include these variable names without secret values.")
        if not hints:
...
class _DetectionState:
    ...
    env_vars_detected: Set[str] = field(default_factory=set)
...
def _parse_env_example_keys(content: str) -> Set[str]:
    keys: Set[str] = set()
...
def _looks_like_runtime_config_path(path: str) -> bool:
    lower = path.lower()
    ...
    if name in {
        ...
        return True
    return any(part in lower for part in ("/config/", "/settings/", "/infra/", "/deploy/"))
